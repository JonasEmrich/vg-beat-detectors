...
the visibility graph based methods provide significantly superior
performance compared to popular R-peak detectors and allow for
sample-accurate R-peak detection.
...
graph and a weighted horizontal visibility graph, respectively.
...
their influence consult the papers \[2\].
...
setting `accelerated` to `False` or `True` respectively. The
acceleration results in a reduced run-time by one magnitude while the
detection accuracy remains comparable with the non-accelerated detector.
...
when changes have been made in the `graph_type` and `edge_weight`
options.
